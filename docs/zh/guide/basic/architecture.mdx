...
具体更多细节将在后面[Live DSL](/guide/basic/live-DSL)章节介绍。
...
同样，本章不会去深入着色器语法和其渲染机制的更多细节，您看查看[着色语言](guide/basic/shading-language)章节介绍。
...
更多细节我们在后面 [Makepad Widget](/guide/basic/widget) 章节中介绍。



## 小结

本章对 `Makepad Framework` 的架构做了基本介绍，描述了 `Makepad Framework` 两大组成部分 `platform` 和 `widget` 各自的作用和基本组成结构，让开发者对 `Makepad Framework` 有整体的了解，为后面进一步使用 makepad 以及深入更多细节做准备 。
